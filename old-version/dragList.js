{
    ...
    class DragList {
        ...
        // @params {number} centerY 当前元素中心的y值
        findIndexAndUpdate(centerY) {
            ...
            if (obj.direction === 'up') {
                // 向上找
                this.lookUpAndMove(curIndex, centerY)
            } else if (obj.direction === 'down') {
                // 向下找
                this.lookDownAndMove(curIndex, centerY)
            } else {
                return
            }
        }

        lookUpAndMove(curIndex, centerY) {
            if (curIndex > limit) {
                let isFind = false
                // 一次查找limit个
                ...
                for (let i = 1; i <= j; i++) {
                    ...
                    for (let k = end; k > start; k--) {
                        // 判断是否进入以及移动
                        let result = this.isEnter(k, centerY)
                        if (result) {
                            isFind = true
                            break
                        }
                ...
                for (let i = curIndex; i >= 0; i--) {
                    let result = this.isEnter(i, centerY)
                    if (result) break
                }
            }
        }

        lookDownAndMove(curIndex, centerY) {
